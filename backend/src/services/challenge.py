...
async def create_challenge(db: db_dependency, user_id: UUID) -> Challenge:
  get_quota = await db.execute(select(User.quota_remaining).where(User.id == user_id))
  quota_remaining = get_quota.scalar_one_or_none()

  if quota_remaining is None or quota_remaining <= 0:
    raise HTTPException(
      status_code=status.HTTP_403_FORBIDDEN,
      detail="Your credits are exhausted. Please wait for the quota to reset."
    )
...
async def get_challenges(db: db_dependency, user_id: UUID):
  ...
  if not challenges:
    raise HTTPException(
      ...
      detail="No challenges found for this user"
    )
